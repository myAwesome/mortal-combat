...
import { getLigue, getLigueRanking, getLiguePlayers, addLiguePlayer, removeLiguePlayer } from '../api/ligues'
...
export default function LigueDetail() {
  ...
  const availablePlayers = allPlayers.filter((p) => {
    // filter out players already in this ligue
    return !liguePlayers.some((lp) => String(lp.id) === String(p.id))
  })

  // Actually we need to check by player_id, but the API returns ligue_player id.
  // Let's use names as fallback or check by name match.
  // The API returns: { id (lp row id), name, points, champs }
  // So we filter by name
  const liguePlayerNames = new Set(liguePlayers.map((lp) => lp.name))
  const filteredAvailable = allPlayers.filter((p) => !liguePlayerNames.has(p.name))
